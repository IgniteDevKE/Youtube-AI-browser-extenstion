...
export const Markdown = ({ markdown, className }: MarkdownProps) => {
  const convertToHtml = (markdownText: string) => {
    ...
    markdownText = markdownText.replace(
      /^> (.*$)/gim,
      '<blockquote class="border-l-4 border-gray-500 pl-4">$1</blockquote>'
    )

    // Convert unordered lists
    markdownText = markdownText.replace(
      /^(?:-|\+|\*)\s+(.+)$/gim,
      '<li class="list-disc ml-5 text-[12px]">$1</li>'
    )
    markdownText = markdownText.replace(
      /((?:<li class="list-disc ml-5">.+<\/li>\s*)+)/gim,
      '<ul class="list-inside">$1</ul>'
    )

    // Convert ordered lists
    markdownText = markdownText.replace(
      /^\d+\.\s+(.+)$/gim,
      '<li class="list-decimal ml-5 text-[12px]">$1</li>'
    )
    markdownText = markdownText.replace(
